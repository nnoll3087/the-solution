    ...
    <div>
      ...
      {formOpen && (
        <div className="bg-surface/80 backdrop-blur rounded-lg border border-border-themed p-5 mb-5 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-text">{editingId ? 'Edit recipe' : 'New recipe'}</h2>
      ...
      {recipes.length === 0 ? (
        <p className="text-sm text-text-subtle">
          {query ? 'No recipes match that search.' : 'No recipes yet. Add your first one above.'}
        </p>
      ) : (
