...
6. New themes are generated by `POST /api/theme/generate`, which sends a detailed system prompt to
   ...
   Particles support `shape: 'custom'` with an AI-drawn `customPath` (SVG path, 24x24 viewBox) for
   theme-specific silhouettes like spiders or rockets. `SceneParticles` validates the path with a
   character allowlist and falls back to a circle if it's malformed.
