...
components/
  Calendar.tsx             # Top-level widget: view switching, nav, event CRUD wiring.
                            # Views: agenda | day | week | month. Defaults to agenda under
                            # 1024px (phones), month on desktop/kiosk.
  AgendaView.tsx           # Rolling 14-day list starting today; the phone-first lookup view
  MonthView.tsx / WeekView.tsx / DayView.tsx   # Week/Day do column-based overlap layout for
                                                ...
                                                # Day shows a compressed 8am-9pm window (expands for
                                                # outliers); Month shows dots-only under sm:
...
6. New themes are generated by `POST /api/theme/generate`, which sends a detailed system prompt to
   ...
   Particles support `shape: 'emoji'` (an emoji character as the floater — the main wow lever),
   `shape: 'custom'` with `customSvg` (multi-element SVG markup) or legacy `customPath` (single
   path). Decorations support `type: 'custom'` corner art (large AI-drawn SVG, e.g. a stroked
   spiderweb). All AI SVG renders through `components/scene/sanitizeSvg.ts`, a strict
   tag/attribute whitelist parser — never render AI markup without it. The generation prompt
   carries theme recipes; the pre-wow "elegant" prompt version lives at commit ef9f63c if the
   user ever wants to revert.
...
- Live on Vercel with Neon Postgres and household password auth (July 2026)
- Rolling agenda view (phone default), compressed day timeline, dense month view with mobile
  dots treatment, responsive header, queue person click-through, AI custom particle shapes
- Custody day coloring: config on `/setup` (`CustodySettings`, stored in the `config` key as
  `custody: {calendarKey, rules[]}`, API `/api/custody`). All-day events on the chosen calendar
  whose titles contain a rule's match text get a `custody` field in `fetchAllEvents`, are hidden
...
- Person filter chips above the calendar (in `Calendar.tsx`): toggle any subset of people;
  ...
  per device in localStorage (`solution-person-filter`).
- Credit countdown in the ThemePrompt popover (`lib/usage.ts`, `/api/usage`) + Zen mode
  (`components/ZenMode.tsx`): ✨ button bottom-right fades out all content so only the themed
  scene shows; tap or any key returns. Intended to double as the kiosk screensaver later
  (idle-timer auto-entry is a future kiosk-polish item).

**Not yet done** (priority order):
1. ~~Theme-token polish~~ — done.
...
3. Kiosk polish for the Raspberry Pi target: fullscreen mode, auto-refresh events every ~5 min,
   touch target sizing, a screen-on/wake strategy. Not started. Deliberately deferred until the
   web/mobile experience is solid.
